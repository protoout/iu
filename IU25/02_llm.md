...
| ツール | 提供元 | 特徴 |
| --- | --- | --- |
| ChatGPT | OpenAI | 対話型LLMの先駆けであり、チャット形式での対話に特化していますがマルチモーダル対応もできます。思考を深めた回答をする推論モデルや、AIが自分でWebサイトを見にいき根拠のある情報を元に回答するDeep Research機能もあります。 |
| Gemini | Google | テキストや画像、音声など複数の形式を統合して同時に処理できるマルチモーダル対応が強みです。Google製品との連携ができるため、GoogleカレンダーやGmaiなど実務でも多く利用されています。 |
| Microsoft Copilot | Microsoft | Word / Excel / TeamsなどのM365製品との連携ができます。実務でも使われることが多く、コンテンツの作成やデータ分析などのタスクの自動化ができます。 |
| Claude | Anthropic | 大量の長文テキストやデータの処理が得意です。人間味のある自然な対話のやりとりができ、倫理的なガイドラインを設けていることから公平で安全な回答を生成することができます。 |
