...
上記のような課題には、主に次の補い方があります。

- **プロンプトの工夫**：指示を整えて品質を上げる
- **RAG**：外部/学内データを**検索**し、その結果を**根拠**に回答させる
- **ファインチューニング**：追加学習で特定領域に強くする
- **外部連携**：外部サービスとつないで最新情報の取得や処理の実行をする

Difyは、外部サービス連携やナレッジの取り込みができ、RAGを組み込んだAIアプリを作りやすいツールなので、今回は **プロンプトの工夫** と **RAG** をハンズオンで体験します。  
...
<details>
  ...
  今回のワークショップでは **RAG** をハンズオンで扱いますが、Difyでは他にも次のような拡張も実現できます。  
