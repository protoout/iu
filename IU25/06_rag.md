...
| やりたいこと | LLM単体 | RAGあり |
|---|---|---|
| 「今日の講義PDFの内容を噛み砕いてほしい」 | 参照できないと「見れません」「推測です」になりがち | **PDFから該当部分を検索**して、要点を短く整理して説明できる |
| 「学内/ゼミのルールを確認したい」 | 一般論でそれっぽく答えがち（根拠が弱い） | **配布資料や規約の該当箇所**を探して、それを根拠に答えられる |
| 「ExcelをPDF変換して送信してほしい」 | 文章で説明して終わりがち（実行はできない） | **手順や必要情報を検索して**、実行のための案内（入力項目・操作手順）まで落とし込める |
...
自分の身近なテーマの資料（大学の案内や施設のFAQなど）を用意してナレッジに登録し、同じ手順でRAGが効くか試しましょう。
