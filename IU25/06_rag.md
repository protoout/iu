...
5. 現状は下記のようにローカルな観光情報が返ってこず、それっぽい嘘の情報が返ってきていてハルシネーションがおきています。  
  - 本来は、成人男性の平均は50-70杯（そば3.5〜4.5人前相当）、女性は30-50杯（2〜3人前相当）程度

    <img src="https://i.gyazo.com/d446e26857d12d8cdcd606fd4d12f716.png" width="450px" alt="image from gyazo"/>
...
### 2-3. ナレッジのチャンク設定をしよう
ここからは検索しやすい形でナレッジデータを保存する設定をしていきます。  
...
1. 次にLLMノードをクリックして右側の画面で設定していきます。
  - 入れるモデルは`Gemini 2.5 Flash-Lite`を選びます。

> [!TIP]
> RAGが効いていることを分かりやすくするため、あえて少し古めのモデルを使います。    
> （賢すぎるモデルだと、RAGなしでも答えてしまうことがあります。）  
>
> <img src="https://i.gyazo.com/b6c11455e24ea06049d4caec1cbb5bcf.png" width="300px" alt="image from gyazo"/>
>
>
...
### 3-1. スクレイピングで外部サイトの最新情報を取り込む
更新頻度が高い情報を扱うには、Webページから都度情報を取得する方法（スクレイピング）が便利です。  
たとえば大学の案内ページなどをHTTPで取得し、必要な部分だけ抽出して回答できます。  
参考：[チャットフロー基本: HTTPノードとIF/ELSEノード](https://zenn.dev/protoout/books/15_dify-manual/viewer/http-if-else)
