...
- 盛岡の情報に詳しいAIチャットを作る（Dify）
- DifyとLINEを連携して、LINE Botとして動かす

このパートでは「何をどの順番でやるのか」を整理し、準備が揃っているか確認します。
...
- 分からない言葉が出てきたら
  - Google検索をしたり、ChatGPTに聞くなど、自分で解決できないかトライしてみましょう  
...
- 用語チェックリスト  
  https://docs.google.com/spreadsheets/d/1IuFcXTJYjVJeuBQK-15VeinFWJonUwvU_vVtmkiNglg/edit?usp=sharing
...
Difyはエージェント的なワークフローやRAG（自社資料を参照して答える仕組み）を、画面上でプロセス設計しながら構築し、アプリとしてデプロイまで進められるオープンソースの開発プラットフォームです。
...
補足: AI活用に迷った場合は、目的（企画）と手段（技術）をセットで考えると良いです。  
例: 今日の仕事減らして ではなく、今日のこの書類仕事をワンクリックで終わらせて のように具体的な目的を設定します。
