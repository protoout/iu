...
このパートでは、RAGを使用して、ローカル情報（盛岡/社内/施設/大学など）にも強いQ&Aチャットボットを作ります。  
生成AIがそれっぽい嘘を返すハルシネーションを減らし、根拠のある回答に寄せるのが目的です。
...
- 前のパートで作った **盛岡の観光アドバイザーチャットbot** がある
- そのチャットが **プレビューで会話できる**状態になっている
